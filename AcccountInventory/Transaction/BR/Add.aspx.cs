    ...
    public partial class Add : System.Web.UI.Page
    {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (ddPA.SelectedValue != "")
            {
                ...
                SqlConnection con = new SqlConnection(connectionString);
                con.Open();
                string query = "INSERT INTO BRParent(Date,AccountID,Description,Ref1,Ref2,TotalDebit,TotalCredit,Difference)"
                ...
                SqlCommand cmd = new SqlCommand(query, con);
                int brParentId = (int)cmd.ExecuteScalar();
                ...
                if (brParentId > 0)
                {
                    ...
                    for (int i = 1; i <= 5; i++)
                    {
                        ...
                        if (ddl.SelectedValue != "")
                        {
                            ...
                            string q = "INSERT INTO BRChild(BRParentID,AccountID,Description,Debit,Credit,Date)"
                              + "VALUES(" + brParentId + "," + accountId + ",'" + desc + "'," + debit + "," + credit + ",'" + pDate.ToString("yyyy-MM-dd hh:mm:ss") + "')";
                            SqlCommand cmd1 = new SqlCommand(q, con);
                            cmd1.ExecuteNonQuery();
                        }
                        if (i == 5)
                        {
                            lblDateError.Text = "Your transaction is saved successfully with voucher # " + brParentId + ", see result in <a href='/Transaction/BR/List.aspx'>Transactions</a>";
