    ...
    public partial class Add : System.Web.UI.Page
    {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (ddPA.SelectedValue != "")
            {
                ...
                SqlConnection con = new SqlConnection(connectionString);
                con.Open();
                string query = "INSERT INTO CRParent(Date,AccountID,Description,Ref1,Ref2,TotalDebit,TotalCredit,Difference)"
                ...
                SqlCommand cmd = new SqlCommand(query, con);
                int crParentId = (int)cmd.ExecuteScalar();
                ...
                if (crParentId > 0)
                {
                    ...
                    for (int i = 1; i <= 5; i++)
                    {
                        ...
                        if (ddl.SelectedValue != "")
                        {
                            ...
                            string q = "INSERT INTO CRChild(CRParentID,AccountID,Description,Debit,Credit,Date)"
                              + "VALUES(" + crParentId + "," + accountId + ",'" + desc + "'," + debit + "," + credit + ",'" + pDate.ToString("yyyy-MM-dd hh:mm:ss") + "')";
                            SqlCommand cmd1 = new SqlCommand(q, con);
                            cmd1.ExecuteNonQuery();
                        }
                        if (i == 5)
                        {
                            lblDateError.Text = "Your transaction is saved successfully with voucher # " + crParentId + ", see result in <a href='/Transaction/CR/List.aspx'>Transactions</a>";
